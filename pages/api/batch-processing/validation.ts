...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    return res.status(200).json({ message: 'Validation stage completed successfully' });
  } catch (error) {
    console.error('[ValidationStage] Error:', error);
    return res.status(500).json({ 
      message: 'Failed to run validation stage',
