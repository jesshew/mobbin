...
export class AccuracyValidationService {
  ...
  private static updateElementsWithValidation(
    ...
    const validatedElements = Array.isArray(validationData) 
      ? validationData 
      : validationData.elements;
      
    if (!Array.isArray(validatedElements)) {
    ...
    validatedElements.forEach((validatedElement: ValidatedElement) => {
      ...
      if (element) {
        ...
        if (validatedElement.accuracy_score < ACCURACY_THRESHOLDS.LOW && validatedElement.suggested_coordinates) {
          ...
          // Mark status as Overwrite for elements below threshold with suggested coordinates
          element.status = 'Overwrite';
        } else if (validatedElement.accuracy_score < ACCURACY_THRESHOLDS.LOW) {
          // Mark status as Overwrite even without suggestions to trigger dashed appearance
          element.status = 'Overwrite';
        }
      }
    });
  ...
  private static async regenerateAnnotatedImage(
    ...
    const coloredElements = elements.map(element => {
      ...
      const enhancedElement = {
        ...element,
        boxColor: color,
        // If status is Overwrite - we'll use suggested coordinates
        // Status is set in updateElementsWithValidation method
        // Consider element hidden if it's not detected
        hidden: element.status === 'Not Detected' || element.status === 'Error',
        // Use dashed style for low accuracy without suggested coordinates  
        dashed: accuracy < ACCURACY_THRESHOLDS.LOW && (!element.suggested_coordinates && element.status === 'Overwrite'),
        // Always mask unless explicitly disabled
        masked: true
      };
